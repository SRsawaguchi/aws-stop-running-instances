...
def main():
    ...
    text = 'Hello, world from Lambda! make'
    ...
    if len(stopped_ec2_instances) > 0:
        text += "*" + str(len(stopped_ec2_instances)) + "個のEC2インスタンスを停止させました*\n"
        for i in stopped_ec2_instances:
            text += " - *" + i + "*\n"
        text += "\n"
    
    if len(stopped_rds_instances) > 0:
        text += "*" + str(len(stopped_rds_instances)) + "個のRDSインスタンスを停止させました*\n"
        for i in stopped_rds_instances:
            text += " - *" + i + "*\n"
        text += "\n"
    
    if len(stopped_ec2_instances) == 0 and len(stopped_rds_instances):
